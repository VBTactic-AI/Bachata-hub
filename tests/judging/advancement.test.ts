...
describe("rolesNotNeedingJudging() — чистая функция", () => {
  ...
  it("не пропускает перетанцовку НА ГРАНИЦЕ ОТСЕВА (SELECT_N) — участников в тай-группе всегда больше мест", () => {
    const skipped = rolesNotNeedingJudging({ LEADER: 3, FOLLOWER: 0 }, 2, false, "TIE_BREAK");
    expect(skipped.size).toBe(0);
  });

  // TIEBREAK-001/A22 (2026-09-07, по жалобе пользователя): перетанцовка ЗА
  // МЕСТО внутри уже прошедших (1-е/2-е место финала) никого не отсеивает —
  // Round.finalistsCount = размер всей тай-группы, участников РОВНО столько
  // же. Решение всё равно вносит HEAD_JUDGE вручную (реордер), поэтому роль
  // корректно пропускается — судьям не нужно бессмысленно отмечать "Да"
  // каждому ("2 из 2"). isFinalStage=true здесь (перетанцовка вставлена
  // сразу после финала) не должно блокировать пропуск, в отличие от самого
  // финала.
  it("пропускает перетанцовку ЗА МЕСТО (FULL_RANK) — участников ровно столько же, сколько мест, даже если isFinalStage=true", () => {
    const skipped = rolesNotNeedingJudging({ LEADER: 2, FOLLOWER: 0 }, 2, true, "TIE_BREAK");
    expect(skipped.has("LEADER")).toBe(true);
  });
