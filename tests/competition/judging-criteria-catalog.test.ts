...
const fakeTx = {
  judgingCriterionCatalog: { create: criterionCreate, findUniqueOrThrow, update: criterionUpdate },
  auditLog: { create: auditCreate },
};

vi.mock("@/lib/prisma", () => ({
  prisma: {
    judgingCriterionCatalog: { findFirst: (...a: unknown[]) => findFirst(...a) },
    $transaction: (fn: (tx: typeof fakeTx) => unknown) => fn(fakeTx),
  },
}));

const { createJudgingCriterionCatalog, updateJudgingCriterionCatalog } = await import(
