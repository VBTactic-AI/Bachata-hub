...
export async function updateDivisionSettings(divisionId: string, input: UpdateDivisionSettingsInput): Promise<void> {
  ...
  // План по этапам — по прямому запросу пользователя (2026-09-09) стал
  // редактируемым через панель категории, но ТОЛЬКО пока для категории ещё
  // не сгенерированы раунды ("категория стартовала" — организатор сам это
  // назвал границей). Уже сгенерированные раунды хранят свой снимок
  // (Round.finalistsCount) — задним числом им это не грозит (CLAUDE.md
  // §50-51), но дальше план применяется только через "Перегенерировать
  // раунды".
  //
  // Вместимость паркета (heatCapacity) — по прямому запросу пользователя
  // (2026-09-10) исключена из этого ограничения: организатору бывает нужно
  // изменить её на уже идущем соревновании (например, реально пришлось
  // сдвинуть границы паркета). Уже созданные раунды/заезды — тоже не
  // затрагиваются задним числом: у Round есть собственное поле heatCapacity
  // (снимок на момент формирования конкретного раунда, draw-engine.ts,
  // `round.heatCapacity ?? round.division.heatCapacity`), не заполняемое
  // явно при обычной генерации — новое значение дивизиона применится только
  // к тем расчётам вместимости, которые ещё не зафиксированы в существующем
  // Round.heatCapacity (например, новые заезды текущего раунда).
