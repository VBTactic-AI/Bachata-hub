...
// же порядок приоритета, что и в авто-доборе при жеребьёвке (fillHelperShortage
// в draw-engine.ts). Порядок (изменён по прямому запросу пользователя,
// 2026-09-09 — было "выше → свои → ниже", после живого теста реального
// соревнования, где во второй заход позвало гостей из категории на два
// уровня выше вместо уже станцевавших своих же): свои, кто уже станцевал в
// другом заезде раунда; если своих нет — ближайшая категория СТРОГО выше
// (если несколько подряд без людей — следующая выше и т.д.); и только если
// и там пусто — ближайшая категория ниже. Это только подсказка для UI —
// организатор может выбрать любого показанного кандидата, ничего не
// блокируется по уровню категории (docs/00_DECISIONS.md, A10).
export async function listHelperCandidates(
  ...
  const suggestedDivision = own ?? higher[0] ?? lower[0] ?? null;
