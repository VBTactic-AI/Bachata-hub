...
async function createTieBreakRoundInTx(
  ...
  const draw = await tx.draw.create({
    data: {
      ...
      reason:
        tie.mode === "FULL_RANK"
          ? `Автоматически сформировано: ничья за места ${tie.startRank}-${(tie.startRank ?? 1) + tie.group.length - 1} в финале (роль ${tie.role === "LEADER" ? "Партнёр" : "Партнёрша"}) — никого не отсеиваем, нужно решить порядок мест.`
          : `Автоматически сформировано: ничья на границе прохода (роль ${tie.role === "LEADER" ? "Партнёр" : "Партнёрша"}).`,
    },
  });
