...
import { updateFestivalFaqItem, deleteFestivalFaqItem, FestivalFaqItemValidationError } from "@/server/events/festival-faq-service";
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string; itemId: string }> }) {
  ...
  try {
    const item = await updateFestivalFaqItem(itemId, user, parsed.data);
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string; itemId: string }> }) {
  ...
  try {
    await deleteFestivalFaqItem(itemId, user);
