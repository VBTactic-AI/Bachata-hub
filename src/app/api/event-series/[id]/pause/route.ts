...
import { pauseEventSeries, EventSeriesForbiddenError, EventSeriesNotFoundError } from "@/server/events/event-series-service";
...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const series = await pauseEventSeries(id, user);
