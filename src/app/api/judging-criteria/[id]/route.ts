...
import { updateJudgingCriterionCatalog, deleteJudgingCriterionCatalog } from "@/server/competition/judging-criteria-catalog";
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  try {
    await deleteJudgingCriterionCatalog(id);
    return NextResponse.json({ ok: true });
  } catch (e) {
    return respondToDomainError(e);
  }
