...
import { setTicketTypeStatus, TicketTypeValidationError } from "@/server/events/ticket-type-service";
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const ticketType = await setTicketTypeStatus(id, user, parsed.data.status);
