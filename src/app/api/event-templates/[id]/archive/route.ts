...
import { archiveEventTemplate, EventTemplateForbiddenError, EventTemplateNotFoundError } from "@/server/events/event-template-service";
...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const template = await archiveEventTemplate(id, user);
