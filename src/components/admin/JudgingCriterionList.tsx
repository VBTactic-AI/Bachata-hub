        ...
        editingId === c.id && draft ? (
          ...
          >
            ...
            <button type="button" onClick={() => startEdit(c)} className="min-w-0 truncate text-left text-sm font-medium text-night-text">
              {c.name} <span className="text-admin-muted font-normal">({c.minScore}–{c.maxScore})</span>
            </button>
            <span className="flex items-center justify-end gap-3">
              <button type="button" onClick={() => hide(c.id)} title="Скрыть" className="text-admin-muted hover:text-night-text" aria-label={`Скрыть критерий ${c.name}`}>
                👁
              </button>
              <button
                type="button"
