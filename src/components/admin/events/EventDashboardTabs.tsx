...
export function EventDashboardTabs({ eventId, canManageTeam }: { eventId: string; canManageTeam: boolean }) {
  ...
  const tabs = [
    ...
    { href: `${base}/passes`, label: "Билеты", exact: false },
    { href: `${base}/statistics`, label: "Статистика", exact: false },
    ...(canManageTeam ? [{ href: `${base}/team`, label: "Команда", exact: false }] : []),
  ];

  return (
    <div role="tablist" aria-label="Разделы события" className="flex gap-1 overflow-x-auto rounded-app border border-admin-border bg-admin-card/50 p-1">
      {tabs.map((tab) => {
