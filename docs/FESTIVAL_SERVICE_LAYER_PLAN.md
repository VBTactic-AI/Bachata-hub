...
**Статус: Stage 1-7 сервисного слоя реализованы, перенос UI (Stage
UI-1..5) тоже завершён** (2026-09-17). Подробности каждого стейджа — в
`docs/PROGRESS.md` (не в git, локальный журнал), здесь только сводка +
решения.

**Перенос UI** (`/admin/festival` — реальная консоль вместо заглушки,
`/festivals/[slug]` — публичная страница) — 5 стадий, каждая закоммичена
отдельно (`311a285`, `c09c143`, `4dbfb4f`, `6dfd88c`, `0f6319f`):
- UI-1: Хаб + Обзор + Программа.
- UI-2: Команда + Пассы — ключевая находка: после появления bridge-Event
  консоль переиспользует готовые компоненты Event-консоли (`PassManager`/
  `PromoCodeManager`/`EventTeamManager`) через `eventSlug`, RBAC совпадает
  1-в-1 (`bridgeEvent.createdById = festival.createdById`).
- UI-3: Спонсоры/FAQ + модерация вопросов гостей/отзывов.
- UI-4: Бюджет + рассылка по Pass (панель на вкладке "Пассы", не отдельная
  вкладка) + информационный чек-лист публикации на "Обзоре".
- UI-5 (финал): публичная `/festivals/[slug]` — потребовала новых
  публичных (без RBAC) функций чтения (`listPublicFestivalSponsors`/
  `listPublicFestivalFaqItems`/`listPublicPassesForEvent`), плюс найден и
  исправлен реальный пробел — bridge-Event создавался без
  `registrationEnabled=true`, поток "гость регистрируется → организатор
  выдаёт Pass" был недостижим с самого Stage 1.

Все стадии UI проверены вживую в браузере на реальных данных (легаси
`Grodno Latina Fest`), не только `tsc`/`build`. **Начиная со Stage UI-2
включительно `git push` не выполняется** — по прямому запросу пользователя,
коммитит/пушит дальше он сам.
